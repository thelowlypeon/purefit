...
- Test fixtures live in `Tests/PureFITTests/Fixtures`, named `<writer>-<sport>-<what it covers>.fit`. Tests reach them through the `Fixture` enum (`try Fixture.garminCycling.pureFITFile()`), never `Bundle.module.url` directly. Adding a fixture means a case there and a row in the Fixtures README.
